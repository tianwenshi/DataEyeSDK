...
@implementation DataEyeExceptionHandler

+ (instancetype)sharedHandler {
    static DataEyeExceptionHandler *gSharedHandler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        gSharedHandler = [[DataEyeExceptionHandler alloc] init];
    });
...
- (void)setupHandlers {
    ...
    NSSetUncaughtExceptionHandler(&DEHandleException);
    
    struct sigaction action;
    sigemptyset(&action.sa_mask);
    action.sa_flags = SA_SIGINFO;
    action.sa_sigaction = &DESignalHandler;
    for (int i = 0; i < sizeof(signals) / sizeof(int); i++) {
        ...
        if (err == 0) {
            ...
            DELogError(@"Errored while trying to set up sigaction for signal %d", signals[i]);
        }
    }
}

static void DEHandleException(NSException *exception) {
    DataEyeExceptionHandler *handler = [DataEyeExceptionHandler sharedHandler];
...
static void DESignalHandler(int signalNumber, struct __siginfo *info, void *context) {
    DataEyeExceptionHandler *handler = [DataEyeExceptionHandler sharedHandler];
...
- (void)td_handleUncaughtException:(NSException *)exception {
    ...
    @try {
        ...
        NSUInteger strMaxLength = DE_PROPERTY_CRASH_LENGTH_LIMIT;
    ...
    DELogInfo(@"Encountered an uncaught exception.");
}
