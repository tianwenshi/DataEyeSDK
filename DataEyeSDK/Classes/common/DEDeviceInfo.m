...
+ (DEDeviceInfo *)sharedManager {
    ...
    static DEDeviceInfo *manager;
    dispatch_once(&onceToken, ^{
        manager = [[DEDeviceInfo alloc] init];
    });
    return manager;
}
...
- (NSDictionary *)getDeviceUniqueId {
    ...
    DEFile *file = [[DEFile alloc] initWithAppid:[DataEyeSDK sharedInstance].appid];
    if (deviceIdKeychain.length == 0 || installTimesKeychain.length == 0) {
