...
@implementation DESqliteDataQueue {
    sqlite3 *_database;
...
+ (DESqliteDataQueue *)sharedInstanceWithAppid:(NSString *)appid {
    static DESqliteDataQueue *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *filepath = [[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DEData-data.plist"];
...
- (id)initWithPath:(NSString *)filePath withAppid:(NSString *)appid {
    ...
    if (sqlite3_open_v2([filePath UTF8String], &_database, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL) == SQLITE_OK ) {
        NSString *_sql = @"create table if not exists DEData (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT, appid TEXT, creatAt INTEGER)";
        char *errorMsg;
...
- (void)addColumn:(NSString *)appid {
    ...
    if (appid.length > 0 && [appid isKindOfClass: [NSString class]])
        query = [NSString stringWithFormat:@"alter table DEData add 'appid' TEXT default \"%@\"", appid];
    else
        query = [NSString stringWithFormat:@"alter table DEData add 'appid' TEXT"];
    NSString *query2 = [NSString stringWithFormat:@"alter table DEData add 'creatAt' INTEGER default %d ", epochInterval];
    char *errMsg;
    @try {
        sqlite3_exec(_database, [query UTF8String], NULL, NULL, &errMsg);
        sqlite3_exec(_database, [query2 UTF8String], NULL, NULL, &errMsg);
    } @catch (NSException *exception) {
        DELogError(@"addColumn: %@", exception);
    }
}

- (BOOL)isExistColumnInTable:(NSString *)column {
    sqlite3_stmt *statement = nil;
    NSString *sql = [NSString stringWithFormat:@"PRAGMA table_info(DEData)"];
...
- (void)delExpiredData {
    ...
    NSDate *date = [[NSDate alloc] initWithTimeIntervalSinceNow: -oneDay * [DEConfig expirationDays]];
...
- (NSArray *)getFirstRecords:(NSUInteger)recordSize withAppid:(NSString *)appid {
    ...
    NSString *query = @"SELECT content FROM DEData where appid=? ORDER BY id ASC LIMIT ?";
...
- (BOOL)removeFirstRecords:(NSUInteger)recordSize withAppid:(NSString *)appid {
    ...
    if (appid.length == 0) {
        query = @"DELETE FROM DEData WHERE id IN (SELECT id FROM DEData ORDER BY id ASC LIMIT ?)";
    } else {
        query = @"DELETE FROM DEData WHERE id IN (SELECT id FROM DEData where appid=? ORDER BY id ASC LIMIT ?)";
    }
...
- (BOOL)removeOldRecords:(int)timestamp {
    NSString *query = @"DELETE FROM DEData WHERE creatAt<?";
...
- (NSInteger)sqliteCountForAppid:(NSString *)appid {
    ...
    NSInteger count = 0;
    if (appid == nil) {
        query = @"select count(*) from DEData";
    } else {
        query = @"select count(*) from DEData where appid=? ";
    }
...
- (void)deleteAll:(NSString *)appid {
    if ([appid isKindOfClass:[NSString class]] && appid.length > 0) {
        NSString *query = @"DELETE FROM DEData where appid=? ";
