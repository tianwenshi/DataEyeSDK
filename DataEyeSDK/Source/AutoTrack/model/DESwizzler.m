...
static void td_swizzledMethod_2(id self, SEL _cmd) {
    ...
    DESwizzle *swizzle = (DESwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
    if (swizzle) {
...
static void td_swizzledMethod_4(id self, SEL _cmd, id arg, id arg2) {
    Method aMethod = class_getInstanceMethod([self class], _cmd);
    DESwizzle *swizzle = (DESwizzle *)[swizzles objectForKey:(__bridge id)((void *)aMethod)];
    if (swizzle) {
...
+ (DESwizzle *)swizzleForMethod:(Method)aMethod {
    return (DESwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
}
...
+ (void)setSwizzle:(DESwizzle *)swizzle forMethod:(Method)aMethod {
...
+ (void)swizzleSelector:(SEL)aSelector
    ...
    if (!aMethod) {
        DELogInfo(@"SwizzleException:Cannot find method for %@ on %@",NSStringFromSelector(aSelector), NSStringFromClass(aClass));
        return;
    }
    
    uint numArgs = method_getNumberOfArguments(aMethod);
    if (numArgs < MIN_ARGS || numArgs > MAX_ARGS) {
        DELogError(@"Cannot swizzle method with %d args", numArgs);
    }
    
    IMP swizzledMethod = (IMP)td_swizzledMethods[numArgs - 2];
    [DESwizzler swizzleSelector:aSelector onClass:aClass withBlock:aBlock andSwizzleMethod:swizzledMethod named:aName];
}
...
+ (void)swizzleSelector:(SEL)aSelector
    ...
    Method aMethod = class_getInstanceMethod(aClass, aSelector);
    if (!aMethod) {
         DELogError(@"Cannot find method for %@ on %@", NSStringFromSelector(aSelector), NSStringFromClass(aClass));
    }
    
    BOOL isLocal = [self isLocallyDefinedMethod:aMethod onClass:aClass];
    DESwizzle *swizzle = [self swizzleForMethod:aMethod];
    ...
    if (isLocal) {
        if (!swizzle) {
            ...
            @try {
                swizzle = [[DESwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
        ...
        if (!class_addMethod(aClass, aSelector, aSwizzleMethod, method_getTypeEncoding(aMethod))) {
             DELogError(@"Could not add swizzled for %@::%@, even though it didn't already exist locally", NSStringFromClass(aClass), NSStringFromSelector(aSelector));
        }
        ...
        if (aMethod == newMethod) {
             DELogError(@"Newly added method for %@::%@ was the same as the old method", NSStringFromClass(aClass), NSStringFromSelector(aSelector));
        }
        
        DESwizzle *newSwizzle = [[DESwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
