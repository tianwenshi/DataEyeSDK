#if __has_include(<DataEyeSDK/DataEyeSDK.h>)
#import <DataEyeSDK/DataEyeSDK.h>
#import <DataEyeSDK/DEDeviceInfo.h>
#else
#import "DataEyeSDK.h"
...
DataEyeSDK* getInstance(NSString *app_id) {
    DataEyeSDK *result = nil;
    ...
    return [DataEyeSDK sharedInstanceWithAppid: app_id];
}
...
void start(const char *app_id, const char *url, int mode, const char *timezone_id) {
    ...
    if (mode == 1) {
        ...
        // DEBUG_ONLY
        config.debugMode = DataEyeDebugOnly;
    }
    ...
    [DataEyeSDK startWithAppId:app_id_string withUrl: url_string withConfig:config];
}

void enable_log(BOOL enable_log) {
    if (enable_log) {
        [DataEyeSDK setLogLevel:DELoggingLevelDebug];
    }
}
...
void set_network_type(int type) {
    switch (type) {
        case NETWORK_TYPE_DEFAULT:
            [[DataEyeSDK sharedInstance] setNetworkType:DENetworkTypeDefault];
            break;
        case NETWORK_TYPE_WIFI:
            [[DataEyeSDK sharedInstance] setNetworkType:DENetworkTypeOnlyWIFI];
            break;
        case NETWORK_TYPE_ALL:
            [[DataEyeSDK sharedInstance] setNetworkType:DENetworkTypeALL];
            break;
    }
}
...
void config_custom_lib_info(const char *lib_name, const char *lib_version) {
    ...
    [DataEyeSDK setCustomerLibInfoWithLibName:lib_name_string libVersion:lib_version_string];
}
...
void track_event(const char *app_id, const char *event_model_json) {
    ...
    DEEventModel *eventModel;
    NSString *eventType = event_model_dic[@"event_type"];
    if ([eventType isEqualToString:@"track_first"]) {
        eventModel = [[DEFirstEventModel alloc] initWithEventName:event_model_dic[@"event_name"] firstCheckID:event_model_dic[@"extra_id"]];
    } else if ([eventType isEqualToString:@"track_update"]) {
        eventModel = [[DEUpdateEventModel alloc] initWithEventName:event_model_dic[@"event_name"] eventID:event_model_dic[@"extra_id"]];
    } else if ([eventType isEqualToString:@"track_overwrite"]) {
        eventModel = [[DEOverwriteEventModel alloc] initWithEventName:event_model_dic[@"event_name"] eventID:event_model_dic[@"extra_id"]];
    }
...
const char *get_device_id() {
    NSString *distinct_id = [[DataEyeSDK sharedInstance] getDeviceId];
    return strdup([distinct_id UTF8String]);
}
...
void create_light_instance(const char *app_id, const char *delegate_app_id) {
    ...
    DataEyeSDK *light = [getInstance(app_id_string) createLightInstance];
...
void enable_autoTrack(const char *app_id, int autoTrackEvents, const char *properties) {
    ...
    NSDictionary *properties_dict = nil;
    convertToDictionary(properties, &properties_dict);
    [[DataEyeSDK sharedInstanceWithAppid:app_id_string] enableAutoTrack:autoTrackEvents];
}
...
void set_autoTrack_properties(const char *app_id, int autoTrackEvents, const char *properties) {
    ...
    NSDictionary *properties_dict = nil;
    convertToDictionary(properties, &properties_dict);
//    [[DataEyeSDK sharedInstanceWithAppid:app_id_string] setAutoTrackProperties: autoTrackEvents properties:properties_dict];
}

const char *get_time_string(const char *app_id, long long time_stamp_millis) {
    NSString *app_id_string = app_id != NULL ? [NSString stringWithUTF8String:app_id] : nil;
    NSDate *time = [NSDate dateWithTimeIntervalSince1970:time_stamp_millis / 1000.0];
    NSString *time_string = [[DataEyeSDK sharedInstanceWithAppid:app_id_string] getTimeString:time];
    return strdup([time_string UTF8String]);
}

void calibrate_time(long long time_stamp_millis) {
    [DataEyeSDK calibrateTime:time_stamp_millis];
}
...
void calibrate_time_with_ntp(const char *ntp_server) {
    ...
    [DataEyeSDK calibrateTimeWithNtp:ntp_server_string];
}
